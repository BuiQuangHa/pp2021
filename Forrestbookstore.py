...
class Books:
    def _init_(self,root):
        ...
        def insert():
            ...
            if (len(BookID.get()) != 0):
                b.insert(BookID.get(), BookName.get(), Price.get(), Author.get(), Year.get(),
                         Publisher.get().Quantity.get())
                BookList.delete(0, END)
                BookList.insert(END, BookID.get(), BookName.get(), Price.get(), Author.get(), Year.get(),
                                Publisher.get().Quantity.get())
        ...
        def search():
            ...
            BookList.delete(0, END)
            for row in b.search(BookID.get(), BookName.get(), Price.get(), Author.get(), Year.get(),
                                Publisher.get().Quantity.get()):
                BookList.insert(END, row, str(""))
        ...
        def update():
            ...
            if (len(BookID.get()) != 0):
                ...
                b.delete(bkr[0])
            if (len(BookID.get()) != 0):
                b.insert(BookID.get, BookName.get(), Price.get(), Author.get(), Year.get(), Publisher.get(),
                         Quantity.get())
                BookList.delete(0, END)
            BookList.insert(END, (
            BookID.get, BookName.get(), Price.get(), Author.get(), Year.get(), Publisher.get(), Quantity.get()))
        ...
        LeftBodyFrame = LabelFrame(Bodyframe, bd=2, width=600, height=350, padx=20, pady=10, bg='green', relief=RIDGE,
                                   font=('arial', 13), text='Details')
        LeftBodyFrame.pack(side=LEFT)

        RightBodyFrame = LabelFrame(Bodyframe, bd=2, width=300, height=350, padx=20, pady=10, bg='green ', relief=RIDGE,
                                    font=('arial', 13), text='Books Information')
...
class Database:
    ...
    def search(self, BookID="", BookName="", Price="", Author="", Year="", Publisher="", Quantity=""):
        ...
        cur = con.cursor()
        cur.execute(
            "select * from Books where BookID=? or BookName=? or Price =? or Author=? or Year=? or Publisher=? or Quantity=?",
            (BookID, BookName, Price, Author, Year, Publisher, Quantity))
        rows = cur.fetchall()
        con.close()
    ...
    def update(self, BookID="", BookName="", Price="", Author="", Year="", Publisher="", Quantity=""):
        ...
        cur = con.cursor()
        cur.execute(
            "update Books set BookID=? or BookName=? or Price =? or Author=? or Year=? or Publisher=? or Quantity=? where BookID=?",
            (BookID, BookName, Price, Author, Year, Publisher, Quantity))
        con.commit()
        con.close()
